...
float picSegment[height][width][3];
scene_t scene;
...
void *threadFunc(void *arg)
{
  ...
  /* for every pixel */
  for( int px=*(int*)arg; px<width; px += globalNthreads )
  {
	...
	scaled_color[i] = max( min(scaled_color[i], 255), 0);
       ...
       */
      for (int i = 0; i < 3; i++)
	picSegment[py][px][i] = scaled_color[i];
    }
  }
  return NULL;
}
...
main( int argc, char **argv )
{
    ...
    // CREATE THREADS AND START INDICES
    pthread_t *threadAddress = (pthread_t*)malloc(sizeof(pthread_t) * nthreads);
    ...
    for (int i = 0; i < nthreads; i++) {
      int retValue = pthread_create(&threadAddress[i], NULL, threadFunc, &numThread[i]);
      if (retValue != 0) {
    ...
    for (int i = 0; i < nthreads; i++) {
      int retValue = pthread_join(threadAddress[i], NULL);
      if (retValue != 0) {
	...
	printf ( "%.0f %.0f %.0f\n",
		 picSegment[j][i][0], picSegment[j][i][1], picSegment[j][i][2] );
      }
      printf("\n");
    }
